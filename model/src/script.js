...
const tick = () => {
  ...
  controls.update();
  if (mixer) {
    const action_index = Math.floor(elapsedTime / 5) % 2 === 0 ? 1 : 2;

    if (action_index !== previousActionIndex) {
      actions[previousActionIndex] && actions[previousActionIndex].stop();
      actions[action_index].play();
    }

    previousActionIndex = action_index;
